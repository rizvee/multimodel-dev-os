...
v4.2 is planned as the next development cycle after the v4.1 Skill OS Foundation release. This document is a planning guide only; v4.2 is not shipped and no gateway runtime exists yet.
...
## 4. Missing Runtime Plane

The runtime plane is not implemented yet.
...
- Provider credential loading and validation.
...
The existing `.ai/models/` files are useful seed metadata, but they are not yet a live router.
...
Security tests should be written before any real provider execution is considered release-ready.

## 11. Revised Sprint Sequence

### Sprint 0: v4.2 Development Lane Safety

- Decide whether `main` should move to `4.2.0-dev.0`.
- Keep this version-only and docs-only if used.
- Confirm v4.1.0 remains npm latest before any version change.
...
- Add fallback planning and simulation.
- Define retry classifications.
- Add rate-limit and timeout policy objects.
...
- Start with mock provider and generic OpenAI-compatible provider only.
...
Start with contracts and pure routing functions before the HTTP server. Keep Skill OS as the control plane, and let the runtime plane consume validated metadata only after the contracts and security model are tested.
