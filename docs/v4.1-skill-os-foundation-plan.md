...
Sprint B adds validation for the Sprint A schema and registry layer. Sprint C adds read-only CLI inspection for that layer. These sprints do not execute automation, enforce permissions, or change existing public command behavior.
...
Sprint A adds the initial schema and example layer for v4.1. Sprint B validates that layer. Sprint C exposes it through read-only inspection:
