...
1. **Build the local tarball archive:**
   ...
   npm pack
   ```
   This creates a file named like `multimodel-dev-os-4.0.0.tgz` in your directory root.
...
2. **Verify bundle contents:**
   ...
   tar -xzf /path/to/multimodel-dev-os-4.0.0.tgz
   ls -la package/
   ```
...
1. Ensure the version in `package.json` is a valid stable major version >= 2 (e.g., v4.0.0).
