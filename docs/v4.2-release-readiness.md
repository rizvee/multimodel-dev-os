...
This document records the readiness assessment for the v4.2 Gateway Foundation. v4.2.0 is prepared for release, but it is not npm-published yet and the GitHub release must remain draft until npm publication is confirmed.
...
- npm publication.
- Public GitHub release finalization.
...
The package retains zero runtime dependencies and package version `4.2.0`. npm publication remains a manual maintainer action. v4.1.0 remains npm `latest` during preparation.
...
No blocker is known for manual npm publication as long as the mock-only and prepared-but-unpublished limitations remain documented.
